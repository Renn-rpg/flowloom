...
**每次完成一个独立任务后，必须启动 2+ 个 Explore 子 agent 从不同角度进行对抗式复审，然后根据复审结果修复发现的问题（至少修复所有高严重度 + 中严重度问题），最后运行全量测试确认零回归。**

具体要求：
1. **复审时机**：每完成一个功能/修复/重构（对应计划中的一个独立项），立即复审，不等批量累积。
2. **复审维度**：至少两个独立角度，例如：
   - 安全性 + 健壮性（边界情况、注入、竞态、错误处理）
   - 代码质量 + 架构（重复代码、依赖方向、类型安全、未使用代码）
   - 如有测试文件变更，增加"测试质量"角度（假阳性、覆盖盲区、mock 保真度）
3. **复审输出**：按严重程度（高/中/低）排序的发现列表，含文件名+行号。
4. **修复标准**：高严重度全部修复，中严重度至少修复，低严重度酌情修复。修复后全量测试必须通过。
5. **死代码清理**：复审中发现的无用代码、未使用导入、死模块一并删除。
6. **Explore agent 配置**：`subagent_type: 'Explore'`，提示词中明确要求读取并对照源文件，给出精确行号。
